    ...
    @csrf
        <div class="row">
            <div class="form-group col-sm-12 col-md-8 col-lg-8">
                ...
                <input type="text" class="form-control" value="{{$tintuc->tenbaiviet}}" name="tenbaiviet" placeholder="Tên tin tức">
            </div>
                    ...
                    <select class="form-control" id="exampleFormControlSelect1" name="chuyennganh">
                        ...
                        @if(isset($CHUYENNGANHS))
                            @foreach($CHUYENNGANHS as $cn)
                                <option value="{{$cn->id}}" {{$tintuc->chuyennganh_id == $cn->id ? "selected='seleted'" : "" }}>{{$cn->tenchuyennganh}}</option>
                            @endforeach
                        @endif
                    </select>
                  </div>
            ...
            <div class="form-group col-sm-12 col-md-4 col-lg-4">
                ...
                @else 
                    <div class="imagePreview" style="background: url('{{asset('storage/uploads/'.$tintuc->avatar)}}'); background-position: center; background-repeat: no-repeat; background-size: cover;"></div>
                @endif
